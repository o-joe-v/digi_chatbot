...
def main():
    ...
    with chat_container:
        for message in st.session_state.messages:
            if message["role"] == "user":
                # User message - orange background, right aligned
                st.markdown(f"""
                <div style="display: flex; justify-content: flex-end; margin: 10px 0;">
                    <div style="background-color: #FFE4B5; padding: 10px; border-radius: 10px; max-width: 70%; text-align: right;">
                        <strong>คุณ:</strong> {message["content"]}
                        <br><small style="color: #666;">⏰ {message.get('timestamp', '')}</small>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                # Assistant message - light blue background, left aligned
                st.markdown(f"""
                <div style="display: flex; justify-content: flex-start; margin: 10px 0;">
                    <div style="background-color: #E6F3FF; padding: 10px; border-radius: 10px; max-width: 70%; text-align: left;">
    ...
    # Input methods - moved below chat history
    st.markdown("---")
